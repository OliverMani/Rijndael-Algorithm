// Rijndael / AES algrímið, notað til að dulkóða gögn
...
const static unsigned char MATRIX[256][6] = {
...
uint32_t subword(uint32_t word) {
	...
	uint32_t result = (int)SBOX[(word >> 4) & 0x0000000F][word & 0x0000000F];
	for(int i = 1; i < 4; i++) {
		result += (int) SBOX[(word >> (((i + 1) * 8) - 4))& 0x0000000F][((word >> (i * 8)))& 0x0000000F] << (i * 8);
	}
	return result;

}

void key_schedule(uint8_t key[], uint32_t w[], int keysize) {
	// r er fyrir umferðir (rounds), og d fyrir dýpt
	int b = 4, r, d;
	uint32_t tmp, rcon[] = { 0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,
		0x40000000,0x80000000,0x1b000000,0x36000000,0x6c000000,0xd8000000,
		0xab000000,0x4d000000,0x9a000000 };
	// Stillir hversu margar umferðir maður á að fara yfir stærð á lyklinum
	switch(keysize){
		case 128:
			r = 10;
			d = 4;
	...
	for(int i = 0; i < d; ++i) {
		for(int j = 0; j < 4; j++) {
			w[i] |= key[4 * i + j] << (24 - (j * 8));
		}
	}

	for(int i = d; i < b * (r + 1); ++i) {
		tmp = w[i - 1];
		if(!(i % d))
			tmp = subword(((tmp << 8) | (tmp >> 24))) ^ rcon[(i - 1) / d];
		else if(d > 6 && (i % d) == 4)
			tmp = subword(tmp);
...
void mix_columns(uint8_t state[][4]){
	...
	for(int i = 0; i < 4; i++) {
		...
		state[0][i] = MATRIX[col[0]][0];
		state[0][i] ^= MATRIX[col[1]][1];
		state[0][i] ^= col[2];
		state[0][i] ^= col[3];
		state[1][i] = col[0];
		state[1][i] ^= MATRIX[col[1]][0];
		state[1][i] ^= MATRIX[col[2]][1];
		state[1][i] ^= col[3];
		state[2][i] = col[0];
		state[2][i] ^= col[1];
		state[2][i] ^= MATRIX[col[2]][0];
		state[2][i] ^= MATRIX[col[3]][1];
		state[3][i] = MATRIX[col[0]][1];
		state[3][i] ^= col[1];
		state[3][i] ^= col[2];
		state[3][i] ^= MATRIX[col[3]][0];
		
	}

}

void add_round_key(uint8_t state[][4], uint32_t w[]) {
	uint8_t subkey[4];
