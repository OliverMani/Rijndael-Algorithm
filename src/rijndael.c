...
const static unsigned char MATRIX[4][4] = {
	{2, 3, 1, 1},
	{1, 2, 3, 1},
	{1, 1, 2, 3},
	{3, 1, 1, 2}
...
unsigned short rcon(unsigned char num) {
	if(num == 1)
		return 1;
	else if(num < 0x80)
		return pow(2, num-1);
	else if(((int)(pow(2, num-1)) ^ 0x11b) > 0xFF)
		return 0;
	return (int)(pow(2, num-1)) ^ 0x11b;
}

void key_schedule(unsigned char* block, int round) {
	unsigned char tmp = block[3];
	for(int i = 1; i < 4; i++) {
		block[((i-1)*4)+3] = block[(i*4)+3];
	}
	block[15] = tmp;
	for(int i = 12; i < 16; i++) {
		int y = block[i] >> 4;
	...
	for(int i = 0; i < 4; i++) {
		W[i] = block[i] ^ block[(i*4)+3] ^ rcon(round);
	}
	for(int x = 0; x < 3; x++) {
		for(int y = 0; y < 4; y++) {
			W[((x+1)*4)+y] = W[(x*4)+y] ^ block[((x+1)*4)+y];
		}
	}
...
void subword(uint32_t word) {
	uint32_t result = (uint32_t) SBOX[(word >> 4) & 0x0000000F][word & 0x0000000F];
	for(int i = 12; i <= 28; i += 8) {
		result += SBOX[(word >> i) & 0x0000000F][(word & 0x0000000F) >> (i - 4)] << (i - 4);
	}
	return result;
...
void mix_columns(uint8_t state[][4]){
	...
	for(int i = 0; i < 4; i++) {
		...
		for(int j = 0; j < 4; j++) {
			state[j][i] = MATRIX[col[0]][0];
			state[j][i]^= MATRIX[col[1]][1];
			state[j][i]^= col[2];
			state[j][i]^= col[3];
		}
	}
...
int main(){
	...
	unsigned char plain[16] = {0x32, 0x88, 0x31, 0xe0, 0x43, 0x5a, 0x31, 0x37, 0xf6, 0x30, 0x98, 0x07, 0xa8, 0x8d, 0xa2, 0x34};
