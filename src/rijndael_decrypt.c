...
void invert_subbytes(uint8_t state[][4])
{
	for(int y = 0; y < 4; y++) {
		for(int x = 0; x < 4; x++) {
			state[y][x] = INV_SBOX[state[y][x] >> 4][state[y][x] & 0xF];
		}
	}
}

void invert_shift_row(uint8_t state[][4])
{
	int t;
...
void invert_mix_columns(uint8_t state[][4])
{
	uint8_t col[4], t;
	for(int i = 0; i < 4; i++) {
		for(int j = 0; j < 4; j++) {
...
void decrypt(uint8_t in[], uint8_t out[], uint32_t key[], int keysize)
{
	...
	add_round_key(state, &key[4*rounds]);
	for(int i = 0; i < rounds; i++) {
		invert_shift_row(state);
		invert_subbytes(state);
		add_round_key(state, &key[4*(rounds-i-1)]);
		if(i != rounds-1)
...
char load_key(char* filename, unsigned char* key) {
...
int main(int argc, char** argv){
	...
	if(!load_key(CONFIG.key_input, key)){
	...
	FILE* out = fopen(CONFIG.output, "wb");
	...
	do {
		...
		decrypt(cipher, plain, i_key_schedule, 256);
		
		fwrite(plain, 1, bread, out);
		if(finish)
			break;
